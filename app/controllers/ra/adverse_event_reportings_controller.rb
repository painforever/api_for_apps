class Ra::AdverseEventReportingsController < ApplicationController
  ...
  def show
    @ad = AdverseEventReporting.find_by_sql("SELECT DISTINCT(adverse_event_reporting.id), adverse_event_reporting.*,
           patient_prescription_items.prescription_id, patient_prescription_items.dosage FROM
           adverse_event_reporting, patient_prescription_items WHERE
           adverse_event_reporting.drug_id = patient_prescription_items.drug_id AND adverse_event_reporting.id = '"+params[:id].to_s+"'")[0]
